...
from exercises.visualizer import print_matrix, draw_markers
...
while rval:
    ...
    if found:
        ...
        K_, roi = cv.getOptimalNewCameraMatrix(K, dist, (width, height), 1, (width, height))
        ...
        img_grid = draw_markers(
            img, xy_grid[:-1, :], color=(0, 255, 0), marker_type=cv.MARKER_CROSS, marker_size=10
        )

        # Visualise
        cv.drawChessboardCorners(img, grid_dims, corners2, found)
        cv.imshow("img", img)
        cv.imshow("img_grid", img_grid)
